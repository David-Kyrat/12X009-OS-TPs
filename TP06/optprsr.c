 ...
 * @return 0 if success otherwise -1 (Invalid argument)
 */
...
int parseArgv(int argc, char* argv[], int isServ, int* port_dest, char** ip_addr_dest) {
    ...
    if (checkArgsErr < 0)  //* print saved errno to sdterr and return -1
        printRErr("%s - %d : Invalid Number of argument(s).\n", argc);

    //* First things first copy received argument to avoid any reference / view-modifying problem
    ...
    int port;
    //* if conversion failed or port is not valid 
    if (strToInt(port_str, 10, &port) < 0 || !isPortValid(port)) {
        free(port_str);
        return -1;
    }
    *port_dest = port;
